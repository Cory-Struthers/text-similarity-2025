...
```
When document lengths were short, position2 and postion4 were least distant than all other pairs (3.00). Now, position2 and position4 (at 5.10) are _more_ distant than position1 and position2 (3.32) and position2 and position3 (3.61).
...
ggdendrogram(news_source_cos_dist_clust, rotate = TRUE) +
  labs(title="Dendrogram of News Articles by Source, Cosine Dissimilarity")
```
...
cos_sim_ss_marriage <- news_source_issue_cos_sim[,grepl( "samesex" , names(news_source_issue_cos_sim) ) ] # columns
cos_sim_ss_marriage <- cos_sim_ss_marriage  %>% 
  filter(grepl("samesex", rownames(cos_sim_ss_marriage))) # rows

cos_sim_tob <- news_source_issue_cos_sim[,grepl( "tobacco" , names(news_source_issue_cos_sim) ) ] # columns
cos_sim_tob <- cos_sim_tob  %>% 
  filter(grepl("tobacco", rownames(cos_sim_tob))) # rows
